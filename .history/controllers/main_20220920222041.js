  ...
  let output = products.reduce((result, product) => {
    ...
    return (
      ...
      <p class="stock">in-stock</p>
          ...
          <i onclick="decrementCart(${+product.id})" class="fa-solid fa-angle-left" data-type="subtract"></i>
          <div id=${+product.id} class="quantity">${
        search.item === undefined ? 0 : search.item
      }</div>
          <i onclick="incrementCart(${+product.id})" class="fa-solid fa-angle-right" data-type="add"></i>
          </div>
          </div>
          </div>
        </div>
